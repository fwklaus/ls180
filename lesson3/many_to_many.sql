...
SELECT DISTINCT books.id, books.author, string_agg(categories.name, ', ') AS categories
FROM books
...
SELECT DISTINCT categories.name, count(books_categories.book_id) AS book_count, string_agg(books.title, ', ') AS book_titles
  FROM books 
  JOIN books_categories
    ON books.id = books_categories.book_id	 
  JOIN categories
